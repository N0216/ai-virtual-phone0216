...
const DEFAULT_ELEVENLABS_MODELS = [
    { id: "eleven_v3", name: "Eleven v3（表现力强）" },
    { id: "eleven_v3_conversational", name: "Eleven v3 Conversational（对话低延迟）" },
    { id: "eleven_multilingual_v2", name: "Eleven Multilingual v2（长文本稳定）" },
    { id: "eleven_flash_v2_5", name: "Eleven Flash v2.5（速度快）" },
];
                                                    ...
                                                    {manualModelIds[config.id] ? (
                                                        <div className="flex gap-2">
                                                            <Input
                                                                ...
                                                                className="flex-1"
                                                            />
                                                            <button type="button" onClick={() => setManualModelIds(prev => ({ ...prev, [config.id]: false }))} className="ui-icon-btn" aria-label="返回模型下拉选择"><List size={20} /></button>
                                                        </div>
                                                    ) : (
                                                        <select
                                                            value={DEFAULT_ELEVENLABS_MODELS.some(m => m.id === config.model) ? config.model : "__manual__"}
                                                            onChange={(e) => {
                                                                if (e.target.value === "__manual__") return setManualModelIds(prev => ({ ...prev, [config.id]: true }));
                                                                updateConfig(config.id, { model: e.target.value });
                                                            }}
                                                            className="ui-select"
                                                        >
                                                            {DEFAULT_ELEVENLABS_MODELS.map(model => <option key={model.id} value={model.id}>{model.name}</option>)}
                                                            <option value="__manual__">手动输入...</option>
                                                        </select>
                                                    )}
                                                    <span className="menu-desc ml-1">Voice ID 在 ElevenLabs 生成声音后复制，粘贴到下方音色栏。</span>
                                                </div>
                                            </>
                                        )}
                                            ...
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <div className="flex gap-2 mt-0.5">
                                                    ...
                                                    >
                                                        ...
                                                        {isFetching[config.id] ? "同步中..." : config.provider === "Minimax" ? "同步音色列表" : config.provider === "ElevenLabs" ? "使用手动 Voice ID" : "显示默认音色"}
                                                    </button>
                                                    {config.provider === "Minimax" && (
                                                        <button
