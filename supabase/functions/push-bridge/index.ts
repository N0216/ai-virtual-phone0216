...
// 现实桥规则和回复快照可能长期驻留；每次真正请求模型前刷新统一时间块。
...
function refreshTemporalAwareness(value: unknown, now = new Date()): void {
  ...
  const safeZone = (raw: string | undefined, fallback = "UTC") => { try { if (raw) new Intl.DateTimeFormat("en-US", { timeZone: raw }).format(now); else throw new Error(); return raw; } catch { return fallback; } };
  const parts = (date: Date, zone: string) => Object.fromEntries(new Intl.DateTimeFormat("en-US", { timeZone: zone, year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).formatToParts(date).map(part => [part.type, part.value]));
  ...
  const rebuild = (attributeText: string) => {
    ...
    try { const members = attrs.group_time_zones ? JSON.parse(decodeURIComponent(attrs.group_time_zones)) : []; const memberRows = Array.isArray(members) ? members.flatMap(member => { const zone = safeZone(typeof member?.timeZone === "string" ? member.timeZone : "", systemZone); return member?.name && zone !== systemZone ? [`${member.name}：${dateTime(now, zone)} ${zone}，${weekday(now, zone)}（${dayPart(now, zone)}）`] : []; }) : []; if (memberRows.length) rows.push("群成员本地时间：", ...memberRows, "判断每个角色作息、问候、深夜/清晨/工作时间时，优先使用该角色自己的本地时间。"); } catch { /* 老快照 */ }
  ...
  for (const key of Array.isArray(value) ? value.keys() : Object.keys(value as Record<string, unknown>)) { const record = value as Record<string | number, unknown>; const item = record[key as string | number]; if (typeof item === "string") record[key as string | number] = item.replace(TEMPORAL_AWARENESS_PATTERN, (_match, attrs: string) => rebuild(attrs)); else refreshTemporalAwareness(item, now); }
}
...
async function callLlm(
  ...
  try {
    const refreshedBody = JSON.parse(bodyJson) as Record<string, unknown>;
    refreshTemporalAwareness(refreshedBody, new Date());
